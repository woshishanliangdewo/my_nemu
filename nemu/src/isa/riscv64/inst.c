...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (R(src1) + imm) & ~(word_t)1; R(rd) = s->pc + 4);
  // INSTPAT("ret");
  // INSTPAT("mv");
  // INSTPAT("j");
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", li     , I, R(rd) = src1 + imm);   // 也即addi
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  // INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc; s->dnpc += imm; R(rd) = s->pc + 4);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal     , J, R(rd) = s->snpc; s->dnpc = s->pc + imm);    // 要更新动态pc
